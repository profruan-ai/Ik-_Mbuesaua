...
export default function App() {
  ...
  const adaptQuestionsWithAI = async (questions: string[]) => {
    try {
      const response = await fetch('/api/adapt-questions', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Falha ao contextualizar questões");
      }
      ...
      throw new Error(`Erro na IA (Groq): ${error.message || "Falha ao contextualizar questões"}`);
    }
  };
